  ...
  <div>
    ...
    <div v-else class="editor-grid">
      ...
      <div class="col-side">
        ...
        <div class="card card--danger">
          ...
          <p class="danger-text">Permanently delete this product. This cannot be undone.</p>
          <button class="btn-danger" @click="showDeleteModal = true">Delete Product</button>
        </div>
    ...
    <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
      <div class="modal">
        ...
        <p class="modal-body">This will permanently remove <strong>{{ form.name }}</strong> from database ledger.</p>
...
let original: Form = emptyForm()

// Local stable keyed arrays to prevent cursor focus loss/auto-hide bugs
...
function populateForm(data: Record<string, unknown>) {
  ...
  const supportRaw = data.support
  let support: string[] = []
  if (Array.isArray(supportRaw)) {
    support = supportRaw.map(String)
  }
  ...
  Object.assign(form, {
    ...
    sub_category: String(data.sub_category ?? ''),
    badge: String((data as Record<string, unknown>).badge ?? ''),
    ...
    stock_status: String(data.stock_status ?? 'active'),
...
onMounted(async () => {
  try {
    const data = await $fetch<Record<string, unknown>>(`/api/admin/products/${productId.value}`)
    populateForm(data)
  } catch {
...
async function handleThumbUpload(e: Event) {
  ...
  try {
    ...
    showToast(err.statusMessage || 'Failed to upload thumbnail', 'error')
  } finally {
    target.value = ''
  }
}
...
async function handleImagesUpload(e: Event) {
  ...
  try {
    ...
    showToast(err.statusMessage || 'Failed to upload gallery image', 'error')
  } finally {
    target.value = ''
  }
}
...
async function saveProduct() {
  ...
  try {
    ...
    const errorMsg = err.data?.statusMessage || err.message || 'Failed to save product'
...
async function doDelete() {
  deleting.value = true
  try {
    await $fetch(`/api/admin/products/${productId.value}?hard=true`, { method: 'DELETE' })
    await router.push(`/${adminPath}/products`)
  } catch {
...
const stockOptions = [
  ...
  { value: 'limited', icon: '⚠️', label: 'Limited', sub: 'Low stock — visible with warning' },
  { value: 'out', icon: '🚫', label: 'Out of Stock', sub: 'Not available right now' },
]
</script>
...
.card--danger { border-color: rgba(239, 68, 68, 0.2); background: rgba(239, 68, 68, 0.01); }
...
.card-title--danger { color: #ef4444; border-color: rgba(239, 68, 68, 0.1); }
...
/* Danger zone */
.danger-text { font-size: 13px; color: #64748b; margin: 0 0 18px; font-weight: 500; line-height: 1.4; }
.btn-danger {
  width: 100%;
  padding: 12px;
...
.modal-title { font-family: 'Outfit', sans-serif; font-size: 22px; font-weight: 900; color: #fff; margin: 0 0 12px; }
.modal-body { font-size: 14px; color: #94a3b8; margin: 0 0 24px; line-height: 1.5; font-weight: 500; }
.modal-actions { display: flex; gap: 12px; justify-content: center; }
.btn-cancel {
  padding: 12px 24px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06);
  border-radius: 12px; color: #cbd5e1; font-size: 13px; font-weight: 700; cursor: pointer; transition: all 0.2s;
}
.btn-cancel:hover { background: rgba(255,255,255,0.06); }
.btn-confirm-delete {
  padding: 12px 24px; background: #ef4444; border: none;
  border-radius: 12px; color: white; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 800; cursor: pointer; transition: all 0.2s;
